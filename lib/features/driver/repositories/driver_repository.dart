...
class DriverRepository {
  ...
  Future<List<Trip>> fetchTripHistory(String driverId) async {
    final rows = await CacheHelper.fetchWithCache(
      table: 'trips',
      onlineFetch: () async {
        final response = await _client.rpc('get_driver_trip_history', params: {
  ...
  Future<Map<String, dynamic>> fetchEarnings(String driverId) async {
    final rows = await CacheHelper.fetchWithCache(
      ...
      onlineFetch: () async {
        final response = await _client
            .from('trips')
            .select('id, fare, driver_cut, created_at')
  ...
  Future<List<TripPassenger>> fetchTripPassengers(String tripId) async {
    final rows = await CacheHelper.fetchWithCache(
      ...
      onlineFetch: () async {
        final response = await _client.rpc('get_trip_passengers', params: {
          'p_trip_id': tripId,
        });
        return (response as List<dynamic>).cast<Map<String, dynamic>>();
      },
  ...
  Future<List<Trip>> fetchScheduledTrips(String driverId) async {
    final rows = await CacheHelper.fetchWithCache(
      table: 'trips',
      onlineFetch: () async {
        final response = await _client.rpc('get_driver_scheduled_trips', params: {
          'p_driver_id': driverId,
        });
        return (response as List<dynamic>).cast<Map<String, dynamic>>();
      },
  ...
  Future<Trip> fetchTripById(String tripId) async {
    final data = await CacheHelper.fetchSingleWithCache(
      ...
      id: tripId,
      onlineFetch: () async {
        final response = await _client.rpc('get_trip_details', params: {
          'p_trip_id': tripId,
        });
